...
const SignInModal: React.FC<SignInModalProps> = ({ openRegisterModal, onClose, openForgotPasswordModal }) => {
  ...
  const handleGoogleAuth = () => {
    window.location.replace(`${config.baseURL}user/auth/google/device?device=web`);
  };

  const handleAppleAuth = () => {
    window.location.replace(`${config.baseURL}user/auth/apple/device?device=web`);
  };
  ...
  return (
    <div className={styles.signInModal}>
        <div className={styles.signInModal__content}>
            ...
            <div className={styles.signInModal__contentEmail}>
                ...
                <div className={styles.signInModal__contentEmailForm}>
                    ...
                    <div className={styles.signInModal__contentEmailFormButton}>
                        <Button value="Sign In" className={styles.signInModal__contentConnectBtn} onClick={handleSubmit} />
                    </div>
                </div>
            </div>
            ...
            <h3>New to GamingArcade? <span onClick={openRegisterModal}>Join Now</span></h3>
        </div>
    </div>
  );
};
